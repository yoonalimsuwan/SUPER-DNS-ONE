...
#
# Changes v6.2 → v6.3:
#   Bug 11 fix: rho_fx/rho_fy/rho_fz (face-interpolated density used in
#     the pressure-projection velocity update) fell back to
#     rho_s.mean() -- the domain-averaged density -- at boundary
#     faces (index 0 and -1), instead of the correct local value.
#     For the default periodic BC this used the wrong density at
#     every wrap-around face on every step; for non-uniform density
#     fields (e.g. Rayleigh-Taylor) it introduces a real bias exactly
#     at the boundaries where the projection matters most. Invisible
#     in the existing test suite because every test there uses a
#     uniform-density Taylor-Green field, where mean() == local value
#     everywhere by construction. Fixed: periodic BC now wraps the
#     boundary face exactly like every other periodic operator in
#     this file; non-periodic BC uses zero-gradient extrapolation
#     from the nearest interior cell.
#
...
#
# Changes v5 → v6:
#   • Import LangevinFHBridge from one_core (Bug 3 fix)
#   • StructuralFluctuatingHydro.step() reads _ext_sigma / _ext_mask
#     injected by LangevinFHBridge for Langevin→FH coupling
#   • ONE_VERSION bumped to 3.0.0
#
...
_SOFTPLUS_B  = 100.0  # softplus sharpness for positivity floors (legacy default)
...
def _softplus_floor(x: torch.Tensor, floor: float,
    ...
    Bug fix (v2): beta previously defaulted to a fixed _SOFTPLUS_B=100,
    giving a transition width of log(2)/beta ~ 6.93e-3 REGARDLESS of
    the requested `floor`. rho_s = _softplus_floor(rho, 1e-12), used
    every timestep in the velocity update (u = rhou/rho_s), was
    confirmed numerically to inflate the effective density floor to
    ~6.93e-3 instead of 1e-12 -- distorting velocity by up to ~6900x
    in any low-density region (rho ~ 1e-6), the same failure mode
    identified and fixed in REAL FOLD ONE / SUPER DNS ONE.
    ...
    Pass an explicit `beta` to opt out for call sites that want the
    old fixed-width behaviour.
    """
    if beta is None:
        width = max(abs(floor), 1e-12)
...
class StructuralFluctuatingHydro(nn.Module):
    ...
    def __init__(self, config: SolverConfig) -> None:
        ...
        # External coupling buffers (written by LangevinFHBridge.sync())
        self.register_buffer(
            "_ext_sigma",
    ...
    def step(
        ...
        # External Langevin coupling: blend external sigma if bridge has synced
        ...
        # Blend with Langevin-projected interface mask if available
        if self._ext_mask.any():
            imask = 0.5 * (imask + self._ext_mask)
        ...
        # Bug 11 fix (v6.3): boundary faces (index 0 and -1) previously
        # fell back to rho_s.mean() -- the domain-averaged density --
        # instead of the correct local value. For the default periodic
        # BC this used the wrong density at every wrap-around face on
        # every step; for non-uniform density (e.g. Rayleigh-Taylor)
        # this introduces a real, physically incorrect bias exactly at
        # the boundaries where the pressure projection matters most.
        # It was invisible in the test suite because every existing
        # test uses a uniform-density Taylor-Green field, where
        # mean() == local value everywhere by construction.
        # Fix: periodic BC wraps the boundary face exactly like every
        # other periodic operator in this file (_pad_field_3d, div_u
        # via roll, etc); non-periodic BC falls back to zero-gradient
        # extrapolation from the nearest interior cell, consistent
        # with the OPEN boundary convention used in _apply_velocity_bc.
        rho_fx = torch.empty(cfg.Nx+1, cfg.Ny, cfg.Nz,
                             device=self.device, dtype=cfg.dtype)
        rho_fy = torch.empty(cfg.Nx, cfg.Ny+1, cfg.Nz,
                             device=self.device, dtype=cfg.dtype)
        rho_fz = torch.empty(cfg.Nx, cfg.Ny, cfg.Nz+1,
                             device=self.device, dtype=cfg.dtype)
        rho_fx[1:-1,:,:] = 0.5*(rho_s[:-1,:,:] + rho_s[1:,:,:])
        rho_fy[:,1:-1,:] = 0.5*(rho_s[:,:-1,:] + rho_s[:,1:,:])
        rho_fz[:,:,1:-1] = 0.5*(rho_s[:,:,:-1] + rho_s[:,:,1:])

        if cfg.bc_x == BoundaryCondition.PERIODIC:
            rho_fx[0,:,:] = rho_fx[-1,:,:] = 0.5*(rho_s[0,:,:] + rho_s[-1,:,:])
        else:
            rho_fx[0,:,:] = rho_s[0,:,:]; rho_fx[-1,:,:] = rho_s[-1,:,:]

        if cfg.bc_y == BoundaryCondition.PERIODIC:
            rho_fy[:,0,:] = rho_fy[:,-1,:] = 0.5*(rho_s[:,0,:] + rho_s[:,-1,:])
        else:
            rho_fy[:,0,:] = rho_s[:,0,:]; rho_fy[:,-1,:] = rho_s[:,-1,:]

        if cfg.bc_z == BoundaryCondition.PERIODIC:
            rho_fz[:,:,0] = rho_fz[:,:,-1] = 0.5*(rho_s[:,:,0] + rho_s[:,:,-1])
        else:
            rho_fz[:,:,0] = rho_s[:,:,0]; rho_fz[:,:,-1] = rho_s[:,:,-1]
...
def _run_tests() -> None:
    ...
    # ── Bug 11 regression: non-uniform density through step() ───────────────
    # Exercises rho_fx/fy/fz with a genuinely non-uniform density field
    # (mean() != local value at the boundary), which the previous test
    # suite never did -- every other step() test uses uniform-density
    # Taylor-Green, where the boundary-mean bug was invisible.
    cfg_rt = SolverConfig(Nx=16,Ny=16,Nz=16,dt=1e-5,
                          enable_fluctuations=False,cfl_limit=0.0,
                          dtype=torch.float64)
    s_rt = StructuralFluctuatingHydro(cfg_rt)
    r_rt,ux_rt,uy_rt,uz_rt,p_rt = s_rt.initialize_rayleigh_taylor()
    try:
        for _ in range(3):
            r_rt,ux_rt,uy_rt,uz_rt,p_rt,d_rt = s_rt.step(r_rt,ux_rt,uy_rt,uz_rt,p_rt)
        if (r_rt.isfinite().all() and r_rt.min()>0
                and ux_rt.isfinite().all() and p_rt.isfinite().all()):
            ok("rayleigh_taylor_3d_step_nonuniform_density")
        else:
            fail("rayleigh_taylor_3d_step_nonuniform_density",
                 "non-finite or non-positive values")
    except Exception as e:
        fail("rayleigh_taylor_3d_step_nonuniform_density", str(e))
