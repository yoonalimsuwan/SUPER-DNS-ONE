...
class InterfaceDetector(InterfaceDetectorBase):  # subclasses ONE Core base
    """
...
class CSOCThermostat(CSOCBase):
    ...
    def forward(
        ...
        # Normalised deviation from target (uses CSOCBase helper)
...
class StructuralItoNoise(StructuralItoBase):
    ...
    def __init__(self, interface_amplification: float = 2.0):
        ...
        # self.amp is set by StructuralItoBase.__init__
    ...
    def get_g_matrix(self, interface_mask: torch.Tensor) -> torch.Tensor:
        ...
        Delegates to :meth:`one_core.StructuralItoBase.get_g_field`.
        """
        return self.get_g_field(interface_mask)
    ...
    def compute_ito_correction(
        ...
        **kwargs,
        ...
        """
        coords = field  # local alias for readability
...
# =============================================================================
# Bridge helper — convenience constructor
# =============================================================================

def make_fh_bridge(integrator, fh_solver, bandwidth: float = 1.0):
    """
    Convenience factory: create a LangevinFHBridge between this integrator
    and a StructuralFluctuatingHydro solver.

    Example::

        bridge = make_fh_bridge(integrator, fh_solver)
        for step in range(n_md):
            coords, velocities, T, sigma = integrator.full_step(...)
            bridge.sync(coords, velocities)     # push state to FH grid
            rho, ux, uy, uz, p, diag = fh_solver.step(rho, ux, uy, uz, p)
    """
    return LangevinFHBridge(integrator, fh_solver, bandwidth=bandwidth)


def make_dns_bridge(integrator, dns_solver):
    """
    Convenience factory: create a LangevinDNSBridge between this integrator
    and a CompressibleSolver.

    Example::

        bridge = make_dns_bridge(integrator, solver)
        for step in range(n_md):
            coords, velocities, T, sigma = integrator.full_step(...)
            bridge.sync(coords)     # push stress signal to DNS solver
            solver.step()
    """
    return LangevinDNSBridge(integrator, dns_solver)
