...
#
# =============================================================================
# WHAT IS IN THIS FILE  (v2.1 vs v2 vs v1)
# =============================================================================
#
# v2.1 NEW / CHANGED
# ===================
# [DOC-1]  Bug 15 note: _step_imex, _step_imex_tf, and _step_pfc_imex all
#          treat the stiff biharmonic-type linear term implicitly using a
#          single scalar sigma value (sigma_bar / M_mean / sigma_mean),
#          not the spatially-varying Structural Laplacian -- exact only
#          for uniform sigma. This was previously undocumented and
#          untested with heterogeneous sigma (the test suite only ever
#          calls scheme='imex' with sigma=ones). Now documented in each
#          method's docstring, and a one-time runtime warning
#          (_warn_if_imex_nonuniform_sigma) fires if IMEX is used with a
#          non-uniform sigma field. scheme='explicit' has no such
#          restriction and remains exact for any sigma.
#
# v2 NEW / CHANGED
# ================
...
def _softplus_floor(x: torch.Tensor, floor: float,
                    ...
                    sharpness: float = 50.0) -> torch.Tensor:
    """
    Differentiable lower bound: floor + softplus(x - floor).

    Bug fix (v2): previously beta defaulted to a fixed _SOFTPLUS_B=100,
    giving a transition width of log(2)/beta ~ 6.93e-3 REGARDLESS of
    the requested `floor`. For any floor << 6.93e-3 (e.g. sigma_min=1e-3
    here, or rho floors of 1e-12/1e-6 elsewhere in the ecosystem), the
    function silently returned ~6.93e-3 instead of the intended floor
    -- confirmed numerically to inflate effective values by 1-7000x.

    This mattered most for degenerate mobility: M(u) = softplus(u)^3
    is designed to vanish (M -> 0) in "dry" regions (u << 0), but
    sigma_eff = _softplus_floor(M*sigma, sigma_min) was clamping those
    regions up to ~6.93e-3 instead of ~1e-3, injecting unintended
    diffusion exactly where the model intends none.

    Fix: beta now scales inversely with floor (beta = sharpness/floor)
    by default, so the transition width tracks the requested floor
    instead of a fixed global constant. An explicit `beta` can still
    be passed to opt out of this scaling for call sites that want the
    old fixed-width behaviour.
    """
    if beta is None:
        width = max(abs(floor), 1e-12)   # avoid beta -> inf for floor=0
        beta = sharpness / width
...
_imex_sigma_warned = False


def _warn_if_imex_nonuniform_sigma(sigma: torch.Tensor, tol: float = 1e-9) -> None:
    """
    Bug 15 note (v2.1): every IMEX scheme in this module (_step_imex,
    _step_imex_tf, _step_pfc_imex) treats the stiff biharmonic-type
    linear term implicitly using a single scalar sigma value (sigma_bar
    or sigma_mean), not the full spatially-varying Structural Laplacian.
    This is mathematically exact only when sigma is uniform: the
    explicit/nonlinear part still uses the correct spatially-varying
    structural operator, so the result is a defensible IMEX
    approximation (treat the constant part implicitly, the rest
    explicitly), but it silently degrades in accuracy for
    heterogeneous sigma fields, since the implicit operator no longer
    matches the true structural one. The 'explicit' scheme (scheme=
    'explicit') has no such restriction and is exact for any sigma.
    Warn once per process so users relying on IMEX with heterogeneous
    sigma are aware, without spamming logs during normal stepping.
    """
    global _imex_sigma_warned
    if _imex_sigma_warned:
        return
    with torch.no_grad():
        if (sigma - sigma.mean()).abs().max().item() > tol:
            logger.warning(
                "[CH3D] scheme='imex' with non-uniform sigma: the IMEX "
                "implicit operator uses a single scalar sigma value for "
                "the stiff linear term and is only exact for uniform "
                "sigma. Accuracy degrades for heterogeneous sigma "
                "fields; use scheme='explicit' if exact structural "
                "weighting of the stiff term is required. "
                "(This warning is shown once per process.)"
            )
            _imex_sigma_warned = True
...
class StructuralCahnHilliard3D(nn.Module):
    ...
    def _step_imex(
        ...
        Caveat (Bug 15 note): the implicit operator above is the plain
        (non-structural) biharmonic, i.e. it implicitly assumes sigma
        is uniform for the stiff term -- only the nonlinear term
        N_hat^n = Delta_S(u^3-u) uses the true spatially-varying
        Structural Laplacian. For uniform sigma this is exact; for
        heterogeneous sigma it is an approximation (see
        _warn_if_imex_nonuniform_sigma). Use scheme='explicit' for an
        IMEX-free integration that is exact for any sigma field.
        """
        _warn_if_imex_nonuniform_sigma(sigma)
        cfg        = self.cfg
        nx, ny, nz = u.shape
...
class ThinFilmStructuralCahnHilliard3D(StructuralCahnHilliard3D):
    ...
    def _step_imex_tf(
        ...
        Caveat (Bug 15 note): the implicit term uses scalar M_mean and
        implicitly assumes uniform sigma; only the explicit nonlin term
        uses the true spatially-varying mobility-weighted Structural
        Laplacian. Exact for uniform sigma; an approximation otherwise
        (see _warn_if_imex_nonuniform_sigma). scheme='explicit' is
        exact for any sigma field.
        """
        _warn_if_imex_nonuniform_sigma(sigma)
        cfg        = self.cfg
        nx, ny, nz = u.shape
...
class PhaseFieldCrystal3D(StructuralCahnHilliard3D):
    ...
    def _step_pfc_imex(
        ...
        Caveat (Bug 15 note): the implicit denominator uses a single
        scalar sigma_mean and substitutes the plain spectral k^4/k^6
        for the second-level Structural Bi-Laplacian (lap2) that
        compute_pfc_chemical_potential computes explicitly elsewhere
        -- exact only for uniform sigma (see
        _warn_if_imex_nonuniform_sigma). scheme='explicit' uses the
        true 3-level Structural Laplacian chain and is exact for any
        sigma field.
        """
        _warn_if_imex_nonuniform_sigma(sigma)
        cfg        = self.cfg
...
if __name__ == "__main__":
    ...
    # 2b. Bug 15 regression: IMEX with non-uniform sigma stays finite
    # and the one-time uniform-sigma warning fires (doesn't raise).
    try:
        cfg2b = CahnHilliardConfig(laplacian="conv3d", scheme="imex",
                                    dt=1e-5, device=device_str)
        ch2b  = StructuralCahnHilliard3D(cfg2b).to(device)
        u2b   = torch.rand(N,N,N,device=device,dtype=torch.float64)*0.1
        sig2b = make_sigma_field(N,N,N, background=1.0,
                                 inclusions=[{"x0":10,"x1":22,"y0":10,"y1":22,
                                              "z0":10,"z1":22,"sigma":5.0}],
                                 device=device_str)
        out2b = ch2b.step(u2b, sig2b)
        assert out2b.isfinite().all()
        ok("imex_nonuniform_sigma_finite_with_warning")
    except Exception as e:
        fail("imex_nonuniform_sigma_finite_with_warning", str(e))
