...
class RealGasEOS:
    ...
    def pressure(self, rho, e):
        ...
        # Move to CPU numpy for CoolProp vectorisation
        rho_np = rho.detach().cpu().numpy().ravel()
        e_np = e.detach().cpu().numpy().ravel()
        # Vectorised CoolProp call (uses loop internally but with numpy interface)
...
class ImmersedBoundary:
    ...
    def __init__(self, mask: torch.Tensor, eta=1e4, u_target=(0.,0.,0.),
        ...
        self.T_target = T_target  # target temperature inside solid (optional)
        self.eta_T = eta_T
        self.gamma = gamma

    def apply_forcing(self, rho, rhou, rhov, rhow, rhoE, dt):
        """Add IB forcing to momentum and energy conserved variables."""
        u = rhou / (rho + 1e-8)
        v = rhov / (rho + 1e-8)
        w = rhow / (rho + 1e-8)
        ...
        # Energy penalty (target temperature)
        if self.T_target is not None:
            # Current temperature: T = p/rho (ideal gas with R=1)
            ke = 0.5 * (rhou**2 + rhov**2 + rhow**2) / (rho + 1e-8)
            p = (self.gamma - 1) * (rhoE - ke)
            T = p / (rho + 1e-8)
            rhoE_target = rho * self.T_target / (self.gamma - 1)  # internal energy only, no kinetic
...
class CompressibleSolver:
    def __init__(self, cfg: CFDConfig):
        ...
        # Wall model helper dictionary for quick lookup of faces with Werner-Wengle
    ...
    def _apply_wall_model(self, rho, rhou, rhov, rhow, rhoE, dt):
        """
        Apply Werner–Wengle wall model source terms to the first interior cell
        adjacent to each wall face. Updates rhou, rhov, rhow in place.
        """
        if not self.wall_model_faces:
            return
        gamma = self.gamma
        dx = self.dx
        # Sutherland's law parameters (same as in RHS)
        T_ref = 300.0
        S = 110.4 / T_ref

        # Face to axis/side mapping
        face_map = {
        ...
        for face in self.wall_model_faces:
            ...
            # Indices of the first interior cell adjacent to the wall
            if axis == 0:
                if side == 'left':
                    i_cell = 1
                    wall_normal_dir = 1  # pointing inward (+x)
                else:
                    i_cell = nx - 2
                    wall_normal_dir = -1
                # Tangential directions are y,z
                # Extract slices
                ...
                # Distance to wall (cell center to wall)
                y = 0.5 * dx
                # Kinematic viscosity at the cell
                ...
                # Werner‑Wengle iterative solve for u_tau
                # Initial guess
                u_tau = torch.sqrt(nu * ut / (y + 1e-8) + 1e-8)
                for _ in range(5):
                    y_plus = y * u_tau / (nu + 1e-8)
                    # Power law: u+ = A * (y+)^B  for y+ > 11.81 else u+ = y+
                    A = self.cfg.wm_A
                    B = self.cfg.wm_B
                    u_plus = torch.where(y_plus <= 11.81, y_plus, A * y_plus**B)
                    u_tau_new = ut / (u_plus + 1e-8)
                    u_tau = 0.5 * (u_tau + u_tau_new)  # under‑relax
                tau_w = rho_slice * u_tau**2
                # Tangential unit vector (v,w)
                ut_mag = ut + 1e-8
                tau_y = -tau_w * (v_slice / ut_mag)  # force opposes motion
                tau_z = -tau_w * (w_slice / ut_mag)
                # Source term per unit volume: tau_w / dx (for cell of size dx)
                src_v = tau_y / dx
                src_w = tau_z / dx
                # Apply to momentum (v,w) of the cell
    ...
    def _compute_rhs(self, rho, rhou, rhov, rhow, rhoE, dt):
        ...
        if self.eos is not None and self.eos.use_real:
            e = (rhoE - ke) / (rho + 1e-8)  # specific internal energy
        ...
        # Recompute pressure in padded domain (ideal gas fallback if real gas not done for padded)
        if self.eos is not None and self.eos.use_real:
    ...
    def step(self, dt=None):
        ...
        # Update pressure, but first diagnose and log if instability arises
        ...
        # Record minimum pressure before clipping for diagnostic
        ...
        # Recompute rhoE consistent with clamped pressure
    ...
    def taylor_green_test(self, steps=200):
        """Run Taylor‑Green vortex and monitor kinetic energy decay."""
        logger.info("=== Taylor–Green vortex test ===")
        self._init_fields('taylor_green')
        self.run(steps=steps)
        # Compute final energy and enstrophy
        u = self.rhou / (self.rho + 1e-8)
        v = self.rhov / (self.rho + 1e-8)
        w = self.rhow / (self.rho + 1e-8)
        ke_final = 0.5 * torch.mean(self.rho * (u**2 + v**2 + w**2)).item()
