# =============================================================================
# SUPER DNS ONE v5 — Native Full-Differentiable 3D Compressible DNS / LES
# =============================================================================
...
class HLLCFlux(RiemannSolverBase):
    ...
    def compute_face_flux(self, rho_pad, u_pad, v_pad, w_pad, p_pad, axis,
        ...
        # SL = min(unL-cL, un_roe-c_roe) → soft min via -logsumexp(-·)
        a1 = unL - cL;  b1 = un_roe - c_roe
        SL = -_logsumexp_max(torch.stack([-a1.reshape(-1), -b1.reshape(-1)], dim=1).T
                              .reshape(2, -1), tau=_TAU_LSE).reshape(a1.shape) \
             if False else 0.5*(a1+b1) - 0.5*_soft_abs(a1-b1)  # smooth min
...
class CSOCKernel(nn.Module):
    """Learnable 5-parameter CSOC kernel — unchanged (already differentiable)."""
...
class SOCController:
    ...
    def nu_t(self, rho, strain_rate_mag, dilatation, dx, dt, c):
        mean_S   = strain_rate_mag.mean() + 1e-8
        ...
        if self.ssc is not None:
            _ = self.ssc(self.stress_acc.mean())
...
class DiffRGRefiner:
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        mask_dc = mask.clone(); mask_dc[0,0,0] = 1.0   # always keep DC

        x_hat_f  = x_hat * mask_dc.to(x_hat.dtype)
        x_filtered = torch.fft.irfftn(x_hat_f, s=(nx,ny,nz))

        # DIFF-FIX 6: smooth rescaling — no Python if-branch on tensor
        mean_after   = x_filtered.mean()
        # softplus ensures denominator ≥ floor without hard branch
        safe_after   = torch.sign(mean_after + 1e-60) * _softplus_floor(
                           mean_after.abs(), 1e-30)
        x_filtered   = x_filtered * (mean_before / safe_after)
        return x_filtered
...
class CompressibleSolver:
    ...
    def _init_bc_objects(self):
        ...
        def _bc(name):
            ...
            if name == 'wall_model':     return WernerWengleWallModelBC(cfg.wm_A,cfg.wm_B)
    ...
    def _apply_wall_model(self, rho, rhou, rhov, rhow, rhoE, dt):
        nu = self.nu_phys; dx = self.dx
        for bc, ax, side in [(self.bc_y_min,1,'left'),(self.bc_y_max,1,'right')]:
            if isinstance(bc, WernerWengleWallModelBC):
                bc.apply(rho, rhou, rhov, rhow, rhoE, ax, side, self.gamma, dx)
    ...
    def save_checkpoint(self, path):
        ...
        ssc_state = (self.soc.ssc._prev.cpu()
                     if (self.soc.ssc is not None and
                         hasattr(self.soc.ssc, '_prev') and
                         self.soc.ssc._prev is not None) else None)
        state = {
            'cfg':         self.cfg,
