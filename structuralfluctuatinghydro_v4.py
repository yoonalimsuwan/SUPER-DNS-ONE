...
# License   : MIT
# Version   : 4.0 (production — full 3D)
# Year      : 2026
#
...
#
# Changes v3 → v4:
#   • Full 3-D upgrade: all fields (Nx,Ny,Nz), MAC faces along all 3 axes
#   • 3-D padding (_pad_field_3d), 7-point Laplacian
#   • rfft3 Poisson solver with 3-D eigenvalue tensor
#   • 3-D interface detector (|∇φ| in xyz)
#   • 3×3 LL stochastic stress tensor (6 independent components)
#   • 3-D advection: upwind / TVD / WENO-5 / semi-Lagrangian (grid_sample)
#   • Rayleigh-Taylor, Taylor-Green, uniform IC all lifted to 3-D
#   • SolverConfig: Nz, Lz, bc_z, dz added
#   • Unit test suite updated for 3-D
#
...
_LIMITERS = {"minmod": _lim_minmod, "van_leer": _lim_van_leer, "superbee": _lim_superbee}
...
def _tvd_flux_1d(
    ...
    flux      = vel_face * (q_upwind + 0.5 * phi * (1.0 - nu.abs()) * dq_dn)
...
def _advect_upwind_3d(
    ...
    drho_dx = torch.where(ucx >= 0.0,
        (rho - fp[:-2, 1:-1, 1:-1]) / dx,
        (fp[2:, 1:-1, 1:-1] - rho) / dx)
    drho_dy = torch.where(ucy >= 0.0,
        (rho - fp[1:-1, :-2, 1:-1]) / dy,
        (fp[1:-1, 2:, 1:-1] - rho) / dy)
    drho_dz = torch.where(ucz >= 0.0,
        (rho - fp[1:-1, 1:-1, :-2]) / dz,
        (fp[1:-1, 1:-1, 2:] - rho) / dz)
...
class LLStochasticStress(nn.Module):
    ...
    def _ito_correction(
        ...
        with torch.enable_grad():
            mask  = detector(rho_d, dx, dy, dz)
            G     = 1.0 + self.amp * mask
            G_sum = G.sum()
            grad_G = torch.autograd.grad(
                G_sum, rho_d, create_graph=False, retain_graph=False
            )[0]
        return (0.5 * G * grad_G).detach()
...
class StructuralFluctuatingHydro(nn.Module):
    ...
    def step(
        ...
        # ── 13. Positivity ──────────────────────────────────────────────────
        rho_new = rho_new.clamp(min=1e-6)
