...
#   • Renormalisation Group (RG) coarse‑graining (3‑D)
#   • Learnable CSOC kernel (adaptive eddy viscosity)
#   • Itô stochastic backscatter (sub‑grid scales, physically consistent)
#   • Batalin–Vilkovisky (BV) consistency checks
#   • Full compressible Navier‑Stokes (hypersonic M > 20)
#   • Adjustable grid size & Reynolds number (Re > 1e8)
#   • Advanced flux splitting: HLLC (robust for all speeds) with MUSCL reconstruction
...
#   • Vendor‑neutral: CPU, Colab T4, Apple MPS, Huawei Ascend,
...
class CSOCKernel(nn.Module):
    ...
    def forward(self, r):
        safe_r = r + self.eps
        # r^{-alpha} * exp(-r/lambda)
        return torch.pow(safe_r, -self.alpha) * torch.exp(-r / self.lambd)
...
class ItoStressGenerator:
    """Itô stochastic stress tensor for sub-grid backscatter."""
    def __init__(self, dt, noise_amp):
        self.dt = dt
        self.noise_amp = noise_amp

    def generate(self, shape, device):
        # generates six independent random stress components (symmetric)
        s11 = self.noise_amp * torch.randn(shape, device=device) * math.sqrt(self.dt)
        s22 = self.noise_amp * torch.randn(shape, device=device) * math.sqrt(self.dt)
        s33 = self.noise_amp * torch.randn(shape, device=device) * math.sqrt(self.dt)
        s12 = self.noise_amp * torch.randn(shape, device=device) * math.sqrt(self.dt)
        s13 = self.noise_amp * torch.randn(shape, device=device) * math.sqrt(self.dt)
        s23 = self.noise_amp * torch.randn(shape, device=device) * math.sqrt(self.dt)
...
class BVFieldTheory:
    ...
    def check_divergence(self, rho, u, v, w, dx):
        # All inputs must be padded (ghost cells)
        rhou = rho * u
        rhov = rho * v
        rhow = rho * w
...
class FluxSplitter:
    """
    Computes inviscid convective fluxes using the HLLC Riemann solver.
    Supports optional MUSCL-Hancock reconstruction with minmod limiter.
    """
    def __init__(self, gamma=1.4, muscl=True, kappa=1.0):
        self.gamma = gamma
        self.muscl = muscl
        self.kappa = kappa  # not used in minmod, kept for compatibility
        self.eps = 1e-8

    def _reconstruct(self, q, axis):
        """
        Return left and right states at cell interfaces using MUSCL (minmod limiter).
        Input q is padded: shape (nx+2, ny+2, nz+2). Output shapes: (nx+1, ny, nz) etc.
        """
        if not self.muscl:
            if axis == 0:
                qL = q[:-1, 1:-1, 1:-1]
        ...
        if axis == 0:
            # Left and right states for each interior interface i+1/2
            q_im1 = q[:-2, 1:-1, 1:-1]   # q_{i-1}
            qi    = q[1:-1, 1:-1, 1:-1]  # q_i
            qip1  = q[2:, 1:-1, 1:-1]    # q_{i+1}

            # Slopes at i (left side) and i+1 (right side)
            slope_i = minmod(qi - q_im1, qip1 - qi)
            slope_ip1 = minmod(qip1 - qi, q[3:, 1:-1, 1:-1] - qip1) if q.shape[0] > 4 else slope_i

            # Reconstructed states at i+1/2:
            qL_int = qi + 0.5 * slope_i
            qR_int = qip1 - 0.5 * slope_ip1

            # Build full interface array (nx+1, ny, nz)
            nx = q.shape[0] - 2
            qL = torch.zeros(nx + 1, q.shape[1] - 2, q.shape[2] - 2, device=q.device)
            qR = torch.zeros_like(qL)
            qL[1:-1] = qL_int   # interior interfaces
            qR[1:-1] = qR_int
            ...
            slope_j = minmod(qj - q_jm1, qjp1 - qj)
            slope_jp1 = minmod(qjp1 - qj, q[1:-1, 3:, 1:-1] - qjp1) if q.shape[1] > 4 else slope_j

            qL_int = qj + 0.5 * slope_j
            qR_int = qjp1 - 0.5 * slope_jp1

            ny = q.shape[1] - 2
            qL = torch.zeros(q.shape[0] - 2, ny + 1, q.shape[2] - 2, device=q.device)
            qR = torch.zeros_like(qL)
            qL[:, 1:-1] = qL_int
            qR[:, 1:-1] = qR_int
            qL[:, 0] = q[1:-1, 0, 1:-1]
            ...
            slope_k = minmod(qk - q_km1, qkp1 - qk)
            slope_kp1 = minmod(qkp1 - qk, q[1:-1, 1:-1, 3:] - qkp1) if q.shape[2] > 4 else slope_k

            qL_int = qk + 0.5 * slope_k
            qR_int = qkp1 - 0.5 * slope_kp1

            nz = q.shape[2] - 2
            qL = torch.zeros(q.shape[0] - 2, q.shape[1] - 2, nz + 1, device=q.device)
            qR = torch.zeros_like(qL)
            qL[..., 1:-1] = qL_int
            qR[..., 1:-1] = qR_int
            qL[..., 0] = q[1:-1, 1:-1, 0]
    ...
    def _hllc_flux(self, rhoL, uL, vL, wL, pL, rhoR, uR, vR, wR, pR, normal):
        ...
        if normal == 0:
            unL = uL; unR = uR; utL = vL; utR = vR; uwL = wL; uwR = wR
        elif normal == 1:
            unL = vL; unR = vR; utL = uL; utR = uR; uwL = wL; uwR = wR
        else:
            unL = wL; unR = wR; utL = uL; utR = uR; uwL = vL; uwR = vR
        ...
        E_face[mask_L] = pL[mask_L] / (gamma - 1.0) + 0.5 * rhoL[mask_L] * (unL[mask_L]**2 + utL[mask_L]**2 + uwL[mask_L]**2)
        ...
        E_face[mask_R] = pR[mask_R] / (gamma - 1.0) + 0.5 * rhoR[mask_R] * (unR[mask_R]**2 + utR[mask_R]**2 + uwR[mask_R]**2)

        # Star state: computed separately for left and right branches
        if mask_star.any():
            ...
            E_starL = p_starL / (gamma - 1.0) + 0.5 * rho_starL * (S_star_s**2 + utL_s**2 + uwL_s**2)

            # Right star region
            factorR = (SR_s - unR_s) / (SR_s - S_star_s + self.eps)
            rho_starR = rhoR_s * factorR
            p_starR = pR_s + rhoR_s * (unR_s - SR_s) * (unR_s - S_star_s)
            E_starR = p_starR / (gamma - 1.0) + 0.5 * rho_starR * (S_star_s**2 + utR_s**2 + uwR_s**2)

            # Which side of contact
    ...
    def compute_flux_divergence(self, rho_pad, u_pad, v_pad, w_pad, p_pad, dx):
        """
        Returns divergence of convective fluxes for interior cells (nx, ny, nz).
        div_flux shape: (nx, ny, nz, 5)
        """
        ...
        for axis in range(3):
            ...
            F_rho, F_u, F_v, F_w, F_E = self._hllc_flux(rhoL, uL, vL, wL, pL,
                                                         rhoR, uR, vR, wR, pR, axis)

            if axis == 0:
                div_flux[..., 0] += (F_rho[1:] - F_rho[:-1]) / dx
                div_flux[..., 1] += (F_u[1:] - F_u[:-1]) / dx
                div_flux[..., 2] += (F_v[1:] - F_v[:-1]) / dx
                div_flux[..., 3] += (F_w[1:] - F_w[:-1]) / dx
                div_flux[..., 4] += (F_E[1:] - F_E[:-1]) / dx
            elif axis == 1:
                div_flux[..., 0] += (F_rho[:, 1:] - F_rho[:, :-1]) / dx
                div_flux[..., 1] += (F_u[:, 1:] - F_u[:, :-1]) / dx
                div_flux[..., 2] += (F_v[:, 1:] - F_v[:, :-1]) / dx
...
class CompressibleSolver:
    def __init__(self, cfg: CFDConfig):
        ...
        self.eps = 1e-8
        self.soc = SOCController(base_temp=cfg.soc_base_temp, friction=0.02,
                                 sigma_target=1.0, use_ssc=True)
        self.ito_gen = ItoStressGenerator(dt=cfg.dt, noise_amp=cfg.ito_noise) if cfg.ito_noise > 0 else None
    ...
    def _pad(self, f):
        return F.pad(f.unsqueeze(0).unsqueeze(0), (1, 1, 1, 1, 1, 1), mode='circular').squeeze()
    ...
    def _init_fields(self, case='taylor_green'):
        ...
        if case == 'taylor_green':
            Mach_eff = max(self.cfg.Mach, 0.1)
            u0 = 0.1 * Mach_eff * 340.0
            ...
            T = torch.ones_like(u) * 300.0 / (self.cfg.gamma * (Mach_eff ** 2 + self.eps))
            p = rho * T
    ...
    def step(self):
        ...
        rho_i = self._unpad(self.rho)
        u_i = self._unpad(self.rhou) / (rho_i + self.eps)
        v_i = self._unpad(self.rhov) / (rho_i + self.eps)
        w_i = self._unpad(self.rhow) / (rho_i + self.eps)
        rhoE_i = self._unpad(self.rhoE)
        ke = 0.5 * rho_i * (u_i ** 2 + v_i ** 2 + w_i ** 2)
        p_i = (gamma - 1.0) * (rhoE_i - ke)
        T_i = p_i / (rho_i + self.eps)

        # Adaptive time step (CFL)
        c = torch.sqrt(gamma * p_i / (rho_i + self.eps))
        max_speed = torch.max(torch.abs(u_i) + c).item()
        dt_cfl = self.cfg.cfl * dx / (max_speed + self.eps)
        dt_used = min(dt, dt_cfl)

        # Physical viscosity
        U_ref = max(abs(u_i.max().item()), abs(v_i.max().item()), abs(w_i.max().item()), 1e-6)
        L_ref = self.cfg.Lx
        nu_phys = U_ref * L_ref / Re if Re > 0 else 0.0

        # SOC eddy viscosity
        ke_local = ke.flatten()
        stress = self.soc.sigma(ke_local)
        nu_t = self._soc_viscosity(stress)
        mu_eff = rho_i * (nu_phys + nu_t)  # shape (nx, ny, nz)

        # Convective flux divergence (HLLC)
        rho_pad = self.rho
        u_pad = self._pad(u_i)
        v_pad = self._pad(v_i)
        w_pad = self._pad(w_i)
        ...
        # Velocity gradients (from padded velocities)
        ...
        # Viscous stress tensor (physical + SOC)
        ...
        # Itô stochastic stress (physically consistent)
        if self.ito_gen is not None:
            shape_int = (self.cfg.nx, self.cfg.ny, self.cfg.nz)
            s11, s22, s33, s12, s13, s23 = self.ito_gen.generate(shape_int, self.device)
            tau_xx += s11
            tau_yy += s22
            tau_zz += s33
            tau_xy += s12
        ...
        # Divergence of viscous + stochastic stress
        ...
        tau_dot_u_x_p = self._pad(tau_dot_u_x)
        tau_dot_u_y_p = self._pad(tau_dot_u_y)
        tau_dot_u_z_p = self._pad(tau_dot_u_z)

        visc_work = (self._deriv(tau_dot_u_x_p, 0, dx) +
                     self._deriv(tau_dot_u_y_p, 1, dx) +
                     self._deriv(tau_dot_u_z_p, 2, dx))
        ...
        qy_p = self._pad(qy)
        qz_p = self._pad(qz)
        heat_flux_div = self._deriv(qx_p, 0, dx) + self._deriv(qy_p, 1, dx) + self._deriv(qz_p, 2, dx)

        visc_rhoE = visc_work + heat_flux_div

        # Update conserved variables (explicit Euler)
        rhou_new = self._unpad(self.rhou) + dt_used * (-conv_rhou + visc_rhou)
        rhov_new = self._unpad(self.rhov) + dt_used * (-conv_rhov + visc_rhov)
        rhow_new = self._unpad(self.rhow) + dt_used * (-conv_rhow + visc_rhow)
        rho_new = rho_i + dt_used * (-conv_rho)
        rhoE_new = rhoE_i + dt_used * (-conv_rhoE + visc_rhoE)

        # Positivity preservation (simple clipping to avoid NaN)
        rho_new = torch.clamp(rho_new, min=1e-6)
        rhoE_new = torch.clamp(rhoE_new, min=1e-6)

        # RG coarse‑graining
        if self.cfg.use_rg and self.rg is not None:
    ...
    def run(self, steps=None):
        ...
        for t in range(steps):
            ...
            if t % 50 == 0:
                ...
                v_pad = self._pad(v_i)
                w_pad = self._pad(w_i)
                div = self.bv.check_divergence(self.rho, u_pad, v_pad, w_pad,
                                               self.cfg.Lx / self.cfg.nx)
    ...
    def grid_convergence_test(self, grid_sizes=[32, 64, 128], ref_steps=50):
        errors = []
        u_ref = None
        original_cfg = self.cfg  # shallow copy, we'll modify and restore
        for N in grid_sizes:
            self.cfg.nx = N
            self.cfg.ny = N
            self.cfg.nz = N
...
def main():
    ...
    parser.add_argument('--rg', action='store_true', help='Enable RG coarse-graining')
    parser.add_argument('--muscl', action='store_true', help='Enable MUSCL reconstruction')
    args = parser.parse_args()

    cfg = CFDConfig(
        nx=args.nx, ny=args.ny, nz=args.nz,
        Re=args.Re, Mach=args.Mach, dt=args.dt, cfl=args.cfl,
    ...
    if args.benchmark:
        ...
        rho_i, u_i, v_i, w_i, _, _ = solver._primitive_from_conserved()
        u_pad = solver._pad(u_i)
        v_pad = solver._pad(v_i)
        w_pad = solver._pad(w_i)
        div = solver.bv.check_divergence(solver.rho, u_pad, v_pad, w_pad,
                                         cfg.Lx / cfg.nx)
