...
SOC‑Controlled compressible Navier–Stokes solver for civilian applications:
  - Medical flows, atmospheric boundary layers, civil aviation
  - AUSM+ and HLLC Riemann solvers + MUSCL reconstruction (2nd order)
  - Self‑Organised Criticality (SOC) adaptive turbulence model
  - Semantic‑State Contraction (SSC) signal denoising & shock capturing
  - Renormalisation Group (RG) via spectral truncation (conservative)
  ...
  - Validation suites: Taylor–Green, shock tube, Kolmogorov spectrum, grid convergence
  - Trainable SOC parameters (5‑parameter differential evolution / Optuna)
...
class CSOCKernel(nn.Module):
    """
    Learnable SOC kernel with 5 parameters:
        Cs     : base Smagorinsky coefficient
        lambda : exponential decay length
        alpha  : power‑law exponent (turbulence intensity sensitivity)
        theta  : critical stress threshold for collapse
        tau    : stress relaxation time
    All parameters are stored in log‑space for unconstrained optimisation.
    """
    def __init__(self,
                 init_Cs: float = 0.18,
    ...
    def forward(self, r: torch.Tensor) -> torch.Tensor:
        """
        r : local normalised strain‑rate intensity (>=0)
        returns local Cs coefficient.
        """
        safe_r = r + 1e-6
...
class SemanticStateContraction:
    """SSC low‑pass filter for signal denoising and shock sensor smoothing."""
...
class SOCController:
    """
    Self‑Organised Criticality turbulence model.
    Combines:
      - learnable local eddy viscosity (Smagorinsky‑like)
      - stress accumulation driven by strain‑rate magnitude
      - relaxation with time constant tau
      - collapse events when stress exceeds threshold theta,
        releasing additional eddy viscosity and resetting stress.
    """
    def __init__(self,
    ...
    def nu_t(self, rho: torch.Tensor, strain_rate_mag: torch.Tensor, dx: float) -> torch.Tensor:
        # Global fluctuation (diagnostic)
        ...
        # Stress accumulation with tau relaxation
        if self.stress_acc is None:
            self.stress_acc = torch.zeros_like(strain_rate_mag)
        dt_acc = 0.01  # accumulation time step (can be linked to solver dt later)
        tau = self.kernel.tau
        dS = strain_rate_mag ** 2 - (1.0 / tau) * self.stress_acc
        self.stress_acc = self.stress_acc + dt_acc * dS
        ...
        # Partial stress release where collapse occurred
...
class DiffRGRefiner:
    """Renormalisation Group (RG) via spectral truncation – conservative filtering."""
...
class BVFieldTheory:
    """Batalin–Vilkovisky inspired diagnostics for conservation laws."""
    @staticmethod
...
# =============================================================================
# 2. Riemann Solvers (AUSM+ and HLLC) with MUSCL Reconstruction
# =============================================================================
class RiemannSolverBase:
    """Shared reconstruction routines for Riemann solvers."""
    @staticmethod
    ...
    def _extract_physical(self, q_pad: torch.Tensor, axis: int, nx: int, ny: int, nz: int):
        """Return physical cell values along axis, shape (N, ...)."""
        if axis == 0:
    ...
    def _muscl_states(self, q_phys: torch.Tensor, order: int) -> Tuple[torch.Tensor, torch.Tensor]:
        """Return left and right states at N+1 interfaces (first dim)."""
        N = q_phys.shape[0]
        if order == 1:
            ...
            s = slope(q_phys)
            left = torch.cat([q_phys[-1:] + 0.5 * s[-1:],
                              q_phys[:-1] + 0.5 * s[:-1]], dim=0)
            right = torch.cat([q_phys - 0.5 * s,
                               (q_phys[:1] - 0.5 * s[:1])], dim=0)
        return left, right
...
class AUSMPlusFlux(RiemannSolverBase):
    ...
    def compute_flux(self,
                     rho_pad: torch.Tensor, u_pad: torch.Tensor,
                     v_pad: torch.Tensor, w_pad: torch.Tensor, p_pad: torch.Tensor,
        ...
        # left/right states at interfaces (first dim = N+1)
        ...
        # permute back to original grid ordering
        if axis == 0:
            return mass_flux, F_rhou, F_rhov, F_rhow, F_rhoE
        elif axis == 1:
            return (mass_flux.permute(1, 0, 2), F_rhou.permute(1, 0, 2),
                    F_rhov.permute(1, 0, 2), F_rhow.permute(1, 0, 2),
                    F_rhoE.permute(1, 0, 2))
        else:
            return (mass_flux.permute(2, 1, 0), F_rhou.permute(2, 1, 0),
                    F_rhov.permute(2, 1, 0), F_rhow.permute(2, 1, 0),
                    F_rhoE.permute(2, 1, 0))
...
# =============================================================================
class CFDConfig:
    """Configuration for the CFD solver."""
    def __init__(self,
...
class CompressibleSolver:
    """SUPER DNS ONE – 3D compressible Navier–Stokes solver."""
    def __init__(self, cfg: CFDConfig):
        ...
        # Conserved fields (padded with 2 ghost cells)
    ...
    def step(self) -> float:
        ...
        # SOC eddy viscosity (includes collapse)
        nu_t = self.soc.nu_t(rho_i, strain_rate_mag, dx)
        mu_eff = mu_lam + rho_i * nu_t

        # Hypersonic shock capturing (SSC‑guided artificial viscosity)
        if self.cfg.shock_capturing:
            ...
            shock_sensor = torch.clamp(-div_v, min=0)  # non‑zero in compression
        ...
        nx, ny, nz = self.cfg.nx, self.cfg.ny, self.cfg.nz
        conv_rho  = torch.zeros(nx, ny, nz, device=self.device)
        ...
        if self.ito_gen is not None:
            self.ito_gen.dt = dt
            s11, s22, s33, s12, s13, s23 = self.ito_gen.generate(tau_xx.shape, self.device)
...
class SignalNoiseSeparator:
    """Denoises sensor data using SSC and optional reference signal."""
...
# =============================================================================
# 5. Trainable SOC (5‑parameter optimisation)
# =============================================================================
class SOCTrainer:
    ...
    def objective(params: np.ndarray, solver: CompressibleSolver,
        ...
        """
        params: [Cs, lambda, alpha, theta, tau]
        """
        solver.soc.kernel.log_Cs.data     = torch.tensor(math.log(params[0]))
        solver.soc.kernel.log_lambda.data = torch.tensor(math.log(params[1]))
        solver.soc.kernel.log_alpha.data  = torch.tensor(math.log(params[2]))
        solver.soc.kernel.log_theta.data  = torch.tensor(math.log(params[3]))
        solver.soc.kernel.log_tau.data    = torch.tensor(math.log(params[4]))
...
def main():
    ...
    parser.add_argument('--shock_capturing', action='store_true',
                        help='Enable SSC‑based shock capturing for hypersonic flows')
