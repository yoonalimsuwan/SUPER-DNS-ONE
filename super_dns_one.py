...
class SemanticStateContraction:
    """
    SSC low-pass (EMA) filter for structural stress σ.

    Changes from v1
    ───────────────
    • ``reset()`` method clears both the accumulated value *and* the
      initialisation flag, so that ``SignalDenoiser`` (and anything else)
      can call ``reset()`` between independent signal windows instead of
      getting a stale EMA that depends on call order.
    • Device-safety: the internal buffer is always moved to the device of
      the incoming tensor, avoiding silent CPU↔GPU copies.
    • The first call seeds the EMA with the input value (no-op output) so
      that the filter does not produce a large transient on initialisation.
    """
    ...
    def __init__(self, epsilon_fp: float = 0.0028, sigma_target: float = 1.0):
        ...
        self._prev: Optional[torch.Tensor] = None   # None = uninitialized

    def reset(self) -> None:
        """Clear EMA state.  Must be called between independent signals."""
        self._prev = None

    def __call__(self, sigma: torch.Tensor) -> torch.Tensor:
        if self._prev is None:
            # Seed: first call is a pass-through; no filtering yet.
            self._prev = sigma.detach().to(sigma.device)
            return sigma
        # Move buffer to the current tensor's device (handles CPU↔GPU migration)
        prev = self._prev.to(sigma.device)
        new  = prev + self.eps * (sigma - prev)
        self._prev = new.detach()
        return new
